...
- **Egress:** Open egress (`egress: - {}`) for dynamic external provider endpoints (OpenStack with arbitrary ports, OCP clusters, certificate retrieval)
- **Critical Ports:** 8443 (API/webhooks), 443 (webhooks), 8444 (certificate service)
- **Note:** Open egress is required because OpenStack deployments use dynamic service discovery with unpredictable ports, and OCP provider validation requires connecting to external clusters with arbitrary endpoints
...
- **Default Services:** Includes standard OpenStack service ports (80, 443, 5000, 8774, 8776, 9696)
- **Custom Ports:** Includes custom OpenStack service ports (13000, 13292, 13774, 13776) for environments with non-standard configurations
- **Service Mapping:**
  - Port 5000: Keystone (Identity service)
  - Port 8774: Nova (Compute service)
  - Port 8776: Cinder (Block Storage service)
...
- **Egress:** Access to OpenStack service APIs (default ports: 80, 443, 5000, 8774, 8776, 9696) and custom ports (13000, 13292, 13774, 13776) for non-standard deployments
- **Authentication:** Uses OpenStack credentials for secure API access
- **Service Coverage:** Supports standard OpenStack deployments and custom port configurations
...
#### **Controller → OpenStack APIs (Open Egress Policy)**
...
**Note:** OpenStack connectivity works through **open egress policy** (`egress: - {}`) for the controller. The OpenStack populator policy includes default OpenStack service ports (80, 443, 5000, 8774, 8776, 9696) and custom ports (13000, 13292, 13774, 13776) to support both standard and non-standard OpenStack deployments.
...
**Result:** ✅ **SUCCESS** - Certificate retrieved and displayed in UI
**Network Policy:** `forklift-api-policy` with open egress (`egress: - {}`) allows arbitrary OpenStack endpoints for certificate retrieval and OCP provider validation
...
- `forklift-api-policy`: API → External Providers (open egress for certificate retrieval and OCP validation - supports arbitrary ports including 443, 13000+)
- `forklift-openstack-populator-policy`: OpenStack Populator → OpenStack APIs (specific ports: 80, 443, 5000, 8774, 8776, 9696, 13000, 13292, 13774, 13776)
...
**Metrics collected**Forklift publishes operational metrics such as:
...
| **OpenStack Provider** | Open Egress (Controller/API) + Specific Ports (Populator) | ✅ Working       | Controller/API: Open egress for arbitrary endpoints. Populator: Ports 80, 443, 5000, 8774, 8776, 9696, 13000, 13292, 13774, 13776 |
...
graph TB
    ...
    OpenStackPop -.->|"Ports 80,443,5000,8774,8776,9696,13000,13292,13774,13776"| OpenStack
